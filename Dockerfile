...
# -----------------------------------------------------------------------------
# Stage 1: 构建 backend Next.js standalone 产物
...
COPY --from=backend-builder /build/.next/standalone ./
# standalone 不含 .next/static 与 public,需手动补齐
COPY --from=backend-builder /build/.next/static ./.next/static

# ---- 2) backend public 静态资源(图标 / svg 等) ----
...
COPY --from=backend-builder /build/drizzle ./drizzle
COPY --from=backend-builder /build/db/migrate.mjs ./db/migrate.mjs
...
COPY --from=backend-builder /build/node_modules/drizzle-orm ./node_modules/drizzle-orm
COPY --from=backend-builder /build/node_modules/postgres ./node_modules/postgres
