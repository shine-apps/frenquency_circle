-- PostGIS 扩展已不再需要:Phase 1 决定改用 lat/lng 双列 + Haversine 公式
-- (见 lib/match/distance.ts),不依赖 ST_DWithin / ST_Distance 等空间函数。
-- 原 `CREATE EXTENSION IF NOT EXISTS postgis;` 已移除,以兼容未安装 PostGIS 的环境。
...
ALTER TABLE "users" ADD COLUMN "privacy_settings" jsonb DEFAULT '{"allowMatch":true,"publicContact":true,"locationPrecision":"exact"}'::jsonb NOT NULL;--> statement-breakpoint
ALTER TABLE "users" ADD COLUMN "practice_years" integer;--> statement-breakpoint
ALTER TABLE "users" ADD COLUMN "activity_level" text DEFAULT 'medium' NOT NULL;--> statement-breakpoint
