...
本文档基于《趣邻圈产品需求文档 (PRD)》编写，面向工程团队，对 V1.0 MVP 的全部功能模块进行页面级拆解。每个页面包含原型线框图、字段规格、交互逻辑和边界条件，确保开发人员可以直接进入实现阶段。
...
| 5 | 匹配结果页 | 双 Tab 展示："同趣的人"和"同趣的圈子"。每个列表项展示头像/标题、距离、兴趣标签、活跃度。支持按距离范围筛选（1/5/10/30km）和排序。点击人项可查看详情并联系，点击圈子项进入圈子详情。 |
...
- 未授权定位：展示引导卡片"请开启定位以发现附近同趣的人"，附带"去设置"按钮
- 附近无数据：展示空状态插画"附近还没有同趣的人，试试扩大搜索范围"
...
- **Tab 切换** `tab`："同趣的人"和"同趣的圈子"两个 Tab，括号内显示匹配数量；切换时加载对应列表
...
点击"联系老师"→ 底部弹窗滑出，展示联系电话（可点击拨号）和微信号（可复制）。弹窗内提示"请说明来自趣邻圈"。后端记录一次"被联系"事件用于统计。若老师未填写任何联系方式，按钮展示为"对方暂未开放联系方式"。
...
| GET | /api/locations/match-people | 匹配同趣的人 | 是 |
| GET | /api/locations/match-circles | 匹配同趣的圈子 | 是 |
...
#### GET /api/locations/match-people — 匹配同趣的人
...
#### GET /api/locations/match-circles — 匹配同趣的圈子
...
> **响应**：
> ```json
> { "code": 0, "data": { "list": [{ "id": 1, "name": "太极拳", "category": "运动健身", "pinyin": "taijiquan" }] } }
> ```
>
...
> 趣邻圈功能设计文档 V1.0 · 基于 PRD 生成 · 2026-06-26
>
