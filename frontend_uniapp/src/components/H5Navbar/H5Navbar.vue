...
const isHomePage = computed(() => currRoute().path === props.homePath)

const userStore = useUserStore()
/** 是否已登录 */
const isLoggedIn = computed(() => userStore.isLoggedIn)
/** 头像 fallback:沿用 me.vue 的逻辑,显示昵称首字 */
const avatarFallback = computed(() => {
  return userStore.userInfo.name ? userStore.userInfo.name[0] : '游'
})

/** 已登录:点击头像/昵称跳转个人中心(tabBar 用 switchTab,否则 navigateTo) */
function goProfile() {
...
<template>
  <view class="box-border [--wot-navbar-bg:#018d71] [--wot-navbar-color:#ffffff] navbar-layout">
    <wd-navbar
      :title="pageTitle"
    ...
    >
      ...
      <template #right>
        <slot name="right">
          <view v-if="isLoggedIn" class="h-8 w-8 flex items-center justify-center overflow-hidden rounded-full bg-white/30" @click="goProfile">
            <image
              ...
              mode="aspectFill"
            />
            <text v-else class="text-sm text-white font-medium">
              {{ avatarFallback }}
            </text>
          </view>
          <view v-else class="border border-white/80 rounded-full px-6 py-2 text-xs text-white" @click="goLogin">
            去登录
          </view>
        </slot>
      </template>
...
<style lang="scss">
// fixed 模式下占位元素同步背景，避免露出下方内容（需 :deep 穿透，无法用原子类替代）
.navbar-layout {
