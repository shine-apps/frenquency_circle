...
const roleInfo = computed<{ text: string; type: 'warning' | 'primary' | 'danger' }>(() => {
  ...
  if (role === 'TEACHER') return { text: '传承人', type: 'warning' }
  if (role === 'ADMIN') return { text: '管理员', type: 'danger' }
...
const roleChipClass = computed(() => {
  if (roleInfo.value.type === 'warning') return 'bg-[#fff7e6] text-[#e68a00]'
  if (roleInfo.value.type === 'danger') return 'bg-[#fff1f0] text-[#ff4d4f]'
...
<template>
  <view class="flex min-h-screen flex-col bg-[#f7f8fa] pb-32">
    ...
    <view class="m-4 flex items-center gap-4 rounded-2xl bg-white p-5" @click="handleProfileClick">
      <view class="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#e8f5f1]">
        <image
          v-if="user?.avatar"
          :src="user.avatar"
          class="h-full w-full"
          mode="aspectFill"
        />
        <text v-else class="text-2xl font-medium text-[#018d71]">
          {{ avatarFallback }}
        </text>
      </view>
      <view class="min-w-0 flex-1">
        <view class="flex items-center gap-2">
          <text class="truncate text-lg font-semibold text-[#333]">
